...
MASTER_FIELDS = {"sector33", "sector17", "market", "scale", "sector33_code", "sector17_code", "scale_code"}
...
def _get_field_value(entry: dict, funda: Optional[dict], field: str):
...
def run_screen(
    ...
    conditions: [{"field": "per", "op": "<=", "value": 15}, ...]
    sort_by: ソートフィールド
    sort_dir: "asc" or "desc"
    limit: 最大件数
    market_filter: 事前フィルタ（市場区分）
    """
    ...
    if not needs_funda and not funda_conditions:
        results = [_build_result(e, None) for e in candidates[:limit * 2]]
        if sort_by:
    ...
    tickers_to_fetch = [e["code_t"] for e in candidates]
    ...
    for entry in candidates[:BATCH_LIMIT]:
        ...
        for cond in funda_conditions:
            val = _get_field_value(entry, funda, cond["field"])
            if not _check_condition(val, cond["op"], cond["value"]):
                all_pass = False
                break

        if all_pass:
            matched.append(_build_result(entry, funda))
...
def _build_result(entry: dict, funda: Optional[dict]) -> dict:
    r = {
        "ticker": entry["code_t"],
        "name": entry["name"],
...
def get_available_fields() -> list[dict]:
    return [
        {"field": k, **v}
        for k, v in SCREENER_FIELDS.items()
    ]
